...
\tableofcontents
\chapter{Sintesi} 
In questa relazione vengono descritte le attività svolte durante il tirocinio riguardanti l'analisi di vulnerabilità web nell'applicazione vulnerabile \textbf{``Juice Shop"}. Vengono illustrate le \textbf{metodologie} utilizzate per identificare e sfruttare le vulnerabilità, nonché le \textbf{contromisure} da adottare per mitigare i rischi associati. Si descrive in dettaglio l'uso dei browser DevTools per l'analisi del codice lato client, l'intercettazione delle richieste HTTP con OWASP ZAP e l'invio di richieste API con Postman. Vengono inoltre presentati i risultati ottenuti nell'identificazione di vulnerabilità come XSS, Injection e Broken Access Control, insieme alle raccomandazioni per migliorare la sicurezza dell'applicazione.
\chapter{Introduzione}
Le vulnerabilità web sono tra le più comuni e pericolose minacce alla sicurezza informatica. Esse possono essere sfruttate da attaccanti per compromettere la riservatezza, l'integrità e la disponibilità dei dati e dei sistemi. Nel corso del tirocinio sono state studiate e analizzate diverse vulnerabilità tra cui XSS (Cross-Site Scripting), Injection e Broken Access Control. L'analisi è stata condotta utilizzando come target l'applicazione web OWASP Juice Shop, mentre per gli strumenti di test sono stati utilizzati OWASP ZAP, Postman e Chrome DevTools.
\par
...
La scheda elementi (figura \ref{fig:chrome-dev-tools-elements} e \ref{fig:api-products-devtools}) mostra l'albero DOM\cite{mdn-dom} della pagina HTML. Consente di ispezionare e modificare gli elementi HTML e i loro stili CSS in tempo reale. Nell'analisi delle vulnerabilità si può trovare utile modificare attributi per consentire operazioni sull'interfaccia grafica (vedi \ref{fig:complaint-form-allowed-extensions}). Inoltre questa visualizzazione consente facilmente di saltare negli \textit{event listener} associati a un elemento come i bottoni, per analizzare il codice JavaScript e cercare endpoint API.
\begin{figure}[H]
\centering
...
\caption{Finestra \textit{Network} con il grafico dei tempi di richiesta, la lista di richieste (login evidenziato) e il Payload}
...
Il Zed Attack Proxy (ZAP) è uno strumento open source per il testing di sicurezza delle applicazioni web. Funziona come un proxy tra il browser e il server web, consentendo di intercettare, modificare e analizzare le richieste e le risposte HTTP. Tra le sue funzionalità principali vi sono:
\begin{itemize}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{images/zap-main-window.png}
\caption{Finestra principale di OWASP ZAP con la lista delle richieste intercettate e il pannello di analisi della richiesta selezionata.}
\label{fig:zap-main-window}
\end{figure}
\subsection{Postman}
Postman è uno strumento popolare per testare e sviluppare API. Consente di creare, inviare e analizzare richieste HTTP in modo semplice e intuitivo. Tra le sue funzionalità principali vi sono:
\begin{itemize}
...
\end{itemize}
Nel corso del tirocinio Postman è stato utilizzato principalmente per inviare richieste \texttt{PUT} e \texttt{POST} alle API del server per testare le vulnerabilità di Broken Access Control e Security Misconfiguration come in figura \ref{fig:api-products-put}
\section{Applicazione vulnerabile}
In rete esistono numerose applicazioni web vulnerabili create appositamente per l'apprendimento e la pratica del testing di sicurezza. Tra queste, \textbf{OWASP Juice Shop} (\ref{fig:juice-shop-logo}) è una delle più popolari e complete. Si tratta di un'applicazione web open source sviluppata da OWASP (Open Web Application Security Project) che simula un negozio online con numerose vulnerabilità comuni.
\begin{figure}[H]
\centering
\includegraphics{images/juice_shop_logo.png}
\caption{Logo dell'applicazione OWASP Juice Shop}
\label{fig:juice-shop-logo}
\end{figure}
\par
La sua Scoreboard consente di tenere traccia delle vulnerabilità scoperte e delle sfide completate e fornisce suggerimenti per risolvere, nonché piccole challenge di sviluppo per trovare la giusta contromisura.
\subsubsection{Installazione}
Per lanciare OWASP Juice Shop sulla macchina locale è necessario  avere installato \textit{Node.js} e \textit{npm}. Successivamente, si può clonare il repository ufficiale da GitHub\cite{juice-shop-github} ed eseguire i seguenti comandi nel terminale: \texttt{npm install} per installare le dipendenze e \texttt{npm start} per avviare l'applicazione. Una volta avviata, l'applicazione sarà accessibile tramite il browser all'indirizzo \texttt{http://localhost:3000}.
\subsubsection{Happy path testing}
Una volta lanciata la applicazione, è stato eseguito il \textit{happy path testing} per comprendere il flusso di lavoro dell'applicazione e identificare le funzionalità principali. Ciò include la navigazione tra le pagine, l'aggiunta di prodotti al carrello, la registrazione e il login degli utenti, nonché il completamento di un ordine. Durante questa fase si è prestato attenzione a eventuali messaggi di errore o comportamenti inaspettati che potrebbero indicare la presenza di vulnerabilità.
\subsubsection{Reconnaisance}
Dopo aver compreso il funzionamento dell'applicazione, il passo successivo è stato la raccolta delle informazioni \textit{information gathering} per scoprire le vulnerabilità. Si è usato ZAP per intercettare le richieste, DevTools per ricercare endpoint API esposti e letti i post, le recensioni degli utenti e i commenti degli sviluppatori per fornire un quadro \textbf{OSINT} (\textit{Open Source Intelligence}).
\par
Questo ha reso possibile la scoperta dell'email dell'amministratore, di alcuni utenti (e delle loro risposte di sicurezza) e chiavi private.
...
\newline
\href{https://owasp.org/www-community/attacks/xss/}{OWASP XSS}\cite{xss-owasp}
\par
Il Cross-Site Scripting viene classificato come una vulnerabilità di tipo Injection, ma viene trattato separatamente per la sua diffusione.
\section{Descrizione}
Il Cross-Site Scripting (XSS) è una vulnerabilità web che consente di lanciare script maliziosi sul browser della vittima che accede a un sito legittimo. Per fare ciò un attaccante può sfruttare diversi metodi tra cui URL manipolati e campi di input non validati.
\par
...
Per prevenire gli attacchi XSS è necessario sanare e validare tutti i dati in input provenienti dagli utenti, sia URL che form. Le metodologie di sviluppo moderne come i framework web ne consentono la mitigazione implementando misure di sicurezza che automaticamente eseguono la sanificazione. Perciò per comprendere bene le potenziali falle di sicurezza è necessario conoscere accuratamente le chiamate dei framework utilizzati. Inoltre, bisogna prestare attenzione all'utilizzo di funzioni che eseguono il \textit{bypass} delle misure di sicurezza come \texttt{bypassSecurityTrust}\cite{angular-domsanitizer} di \textit{Angular}. 
...
\end{figure}
Escogito un payload diverso che sfrutta questa vulnerabilità per eseguire uno script \textit{JavaScript}, che nella realtà si connetterebbe a un server controllato dall'attaccante per rubare i dati dell'utente. Immetto nella barra di ricerca il seguente codice: \texttt{<script>alert(xss)</script>}, ma non verifico la sua esecuzione. Ciò è dovuto presumibilmente al fatto che lo standard \textit{HTML5} non consente l'esecuzione di tag \texttt{<script>} inseriti con \textit{innerHTML}\cite{innerHTML}.
\par
Tento un payload alternativo che utilizza il tag \texttt{<iframe>} per caricare codice esterno: \texttt{<iframe src="javascript:alert(`xss`)">}. In questo modo, il codice viene eseguito correttamente come mostrato in figura \ref{fig:dom-xss-result}. Verificando questa vulnerabilità, l'attaccante può scegliere di inviare link appositamente creati alle vittime per eseguire codice malevolo nei loro browser da una fonte attendibile come il sito web compromesso. Essendo inoltre una vulnerabilità basata sul DOM, il server non ha modo di rilevare l'attacco.
\begin{figure}[H]
\centering
...
\par
Il passo successivo è quello di scoprire le tabelle del database. Ricercando su internet si trova che la query per ottenere i nomi delle tabelle in SQLite è: \texttt{SELECT sql FROM sqlite\_schema}\cite{sqlite-schema-query}. Ricomponendo i payload precedentemente descritti, il payload finale diventa: \texttt{')) UNION SELECT sql FROM sqlite\_master--}. Ora però si tenta di eseguirlo via browser, ma il server restituisce l'errore illustrato in figura \ref{fig:api-search-schema-fail}.
\begin{figure}[H]
\centering
...
\chapter{Conclusioni}
Le vulnerabilità web sono molto diffuse e spesso facili da sfruttare. Derivano principalmente da un uso erroneo delle tecnologie web e da una scarsa conoscenza delle misure di sicurezza da adottare. È fondamentale per gli sviluppatori web essere consapevoli delle potenziali minacce e utilizzare le migliori pratiche di sicurezza per proteggere le applicazioni e i dati degli utenti. Queste possono variare da semplici accorgimenti come la validazione dell'input e l'uso di framework sicuri, fino a misure più avanzate come l'implementazione di sistemi di autenticazione robusti e la crittografia dei dati sensibili. Inoltre, è importante mantenere le applicazioni aggiornate con le ultime \textbf{patch di sicurezza} e monitorare costantemente il traffico per rilevare eventuali attività sospette.
\par
In più, l'implementazione di misure come \textit{rate limiting} e \textit{logging} delle attività può aiutare a prevenire e rilevare attacchi di forza bruta e altre minacce. Infine, è essenziale eseguire regolarmente test di sicurezza e audit per identificare e correggere le vulnerabilità prima che possano essere sfruttate dagli attaccanti.
\printbibliography
\end{document}